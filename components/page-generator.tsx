...
export function PageGenerator({
  ...
  const handleGeneratePage = async () => {
    ...
    if (!storySummary.trim() || !artStyle.trim()) {
      toast.warning("For best results, please define your story and art style first.");
    }
    
    setIsLoading(true);
    toast.info(`Generating page ${currentPageNumber}... This may take a moment.`);
    ...
    try {
      ...
      const characterNames = selectedAssets.filter(a => a.type === 'character').map(c => c.name).join(', ') || 'none';
      const environmentNames = selectedAssets.filter(a => a.type === 'environment').map(e => e.name).join(', ') || 'none';
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
              {characters.map(char => (
                <div key={char.id} className="flex flex-col items-center gap-2">
                  <label htmlFor={char.id} className="cursor-pointer relative">
                    <Image src={char.imageUrl} alt={char.name} width={80} height={80} className="rounded-md object-cover aspect-square border" />
                    <Checkbox id={char.id} onCheckedChange={() => handleAssetSelection(char.id)} checked={selectedAssetIds.has(char.id)} className="absolute top-1 right-1 bg-background" disabled={isLoading} />
                  </label>
             ...
             <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
              {environments.map(env => (
                <div key={env.id} className="flex flex-col items-center gap-2">
                  <label htmlFor={env.id} className="cursor-pointer relative">
                    <Image src={env.imageUrl} alt={env.name} width={80} height={80} className="rounded-md object-cover aspect-square border" />
                    <Checkbox id={env.id} onCheckedChange={() => handleAssetSelection(env.id)} checked={selectedAssetIds.has(env.id)} className="absolute top-1 right-1 bg-background" disabled={isLoading} />
                  </label>
