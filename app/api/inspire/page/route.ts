...
export async function POST(req: Request) {
  try {
    const { storySummary, characterNames, environmentNames, previousPagePrompts } = await req.json();

    if (!storySummary) {
      return NextResponse.json({ error: "Missing story summary." }, { status: 400 });
    }
    ...
    const prompt = `
      ...
      ${previousPagePrompts.map((p: string, i: number) => `${i+1}. ${p}`).join('\n') || 'This is the first page.'}
    ...
    return NextResponse.json({ error: "Failed to generate page idea." }, { status: 500 });
  }
}
